...
uint32_t get_voicefile_addr(uint16_t file_id, uint32_t *p_file_addr, uint32_t *p_file_size)
{
    if ((NULL == p_file_addr) ||(NULL == p_file_size))
    {
        return 1;
    ...
    if (get_file_addr(partition_table.voice_offset, file_id, p_file_addr, p_file_size))
    {
        (*p_file_addr) += partition_table.voice_offset;
        return 0;
    }
