...
{
    if(pdFAIL == xSemaphoreTake(flash_ctl_xSemaphore,pdMS_TO_TICKS(5000)))
    {
        FMO_CI_ASSERT(0,"\n");
    }
}
...
{
    EventBits_t ret = xEventGroupWaitBits(flash_req_op_done_event_group, op_type, pdTRUE, pdFALSE, 3000);
    if(ret != op_type)
    {
        //等待flash信号量超时
        // mprintf("op_type = %d\n",op_type);
