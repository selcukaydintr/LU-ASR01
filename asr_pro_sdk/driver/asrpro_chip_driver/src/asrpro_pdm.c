...
#include "asrpro_pdm.h"
#include "asrpro_alc.h"
...
#define USE_asrpro_OWN_ALC 1//目前没有用到CI110x自带的ALC，为了省代码，用这个宏可以不编译alc.c
...
void pdm_alc_left_config(pdm_alc_use_config_t* ALC_str)
{
    ...
    else if((PDM_GATE_ENABLE == ALC_str->alc_gate) && (PDM_GATE_ENABLE == ALC_str->use_asrpro_alc))
    ...
    if(PDM_GATE_ENABLE == ALC_str->use_asrpro_alc)
    {
        #if USE_asrpro_OWN_ALC
